...
const CreateCourseForm = ({ onCreateCourse, setView }) => {
  ...
  const validateForm = () => {
    ...
    if (formData.deliveryMode === 'OFFLINE' && !formData.venue.trim()) {
      newErrors.venue = "Venue is required for offline courses";
    }
    if (formData.deliveryMode === 'ONLINE' && !formData.meetingLink.trim()) {
      newErrors.meetingLink = "Meeting link is required for online courses";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        createdBy: "admin-id-here", // Replace with actual admin ID from context
      };
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label htmlFor="incharge" className="block text-sm font-medium text-gray-700">Course Coordinator</label>
              <input
                type="text"
                id="incharge"
                name="incharge"
